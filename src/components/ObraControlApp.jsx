...
import { Calendar, Users, Truck, BarChart3, Save, Send, MapPin, TrendingUp, Package, Database, Copy, LogOut, Loader2 } from 'lucide-react';
...
const ObraControlApp = () => {
    ...
    try {
      ...
      if (!projects) {
        const { data: newProject, error: createError } = await supabase
          ...
          .insert([
            {
              name: 'Villa Marina Fase 4',
              client: 'Grupo VerdeAzul',
              created_by: user.id
            }
          ])
          .select()
        ...
        await supabase.from('contract_items').insert(
          partidasContrato.map(p => ({
            project_id: projects.id,
            code: p.codigo,
            description: p.descripcion,
            unit: p.unidad,
            budget_amount: p.presupuesto
          }))
        );
      }
    ...
    try {
      ...
      if (report) {
        ...
        setReportData({
          ...
          personalCampo: report.report_personnel
            .filter(p => p.category === 'field')
            .map(p => ({
              id: p.id,
              nombre: p.name,
              tipo: p.type,
              horasNormales: Number(p.hours_normal),
              horasExtras: Number(p.hours_extra),
              tarifaHora: Number(p.hourly_rate),
              presente: p.is_present,
              actividad: p.activity
            })),
          personalAdmin: report.report_personnel
          ...
          equipos: report.report_equipment.map(e => ({
            id: e.id,
            tipo: e.equipment_type,
            identificacion: e.identification,
            operador: e.operator,
            horasTrabajo: Number(e.hours_worked),
            combustible: Number(e.fuel_consumed),
            costoHora: Number(e.cost_per_hour),
            estado: e.status,
            actividad: e.activity
          })),
          materiales: report.report_materials.map(m => ({
            id: m.id,
            tipo: m.material_type,
          ...
          avancePartidas: report.report_progress.map(p => ({
            id: p.id,
            codigo: p.item_code,
            avanceHoy: Number(p.progress_today),
            avanceAcumulado: Number(p.progress_accumulated),
            observaciones: p.notes
          })),
          observaciones: report.observations || '',
          incidentesSeguridad: report.safety_incidents || ''
        });
      } else {
        setReportId(null);
        setReportData({
  ...
  const saveReport = async () => {
    ...
    try {
          ...
          .insert([
            {
              ...
              weather: reportData.clima,
              observations: reportData.observaciones,
              safety_incidents: reportData.incidentesSeguridad,
              created_by: user.id
            }
          ])
          .select()
      ...
      await supabase.from('report_personnel').delete().eq('report_id', currentReportId);
      await supabase.from('report_equipment').delete().eq('report_id', currentReportId);
      await supabase.from('report_materials').delete().eq('report_id', currentReportId);
      await supabase.from('report_progress').delete().eq('report_id', currentReportId);

      if (reportData.personalCampo.length > 0) {
        await supabase.from('report_personnel').insert(
          reportData.personalCampo.map(p => ({
            report_id: currentReportId,
            name: p.nombre,
            type: p.tipo,
            category: 'field',
            hours_normal: p.horasNormales,
            hours_extra: p.horasExtras,
            hourly_rate: p.tarifaHora,
            is_present: p.presente,
            activity: p.actividad
          }))
        );
      }

      if (reportData.personalAdmin.length > 0) {
        await supabase.from('report_personnel').insert(
          reportData.personalAdmin.map(p => ({
            report_id: currentReportId,
            name: p.nombre,
            type: p.tipo,
            category: 'admin',
      ...
      if (reportData.equipos.length > 0) {
        await supabase.from('report_equipment').insert(
          reportData.equipos.map(e => ({
            report_id: currentReportId,
            equipment_type: e.tipo,
            identification: e.identificacion,
            operator: e.operador,
            hours_worked: e.horasTrabajo,
            fuel_consumed: e.combustible,
            cost_per_hour: e.costoHora,
            status: e.estado,
      ...
      if (reportData.materiales.length > 0) {
        await supabase.from('report_materials').insert(
          reportData.materiales.map(m => ({
            report_id: currentReportId,
            material_type: m.tipo,
            quantity: m.cantidad,
            unit: m.unidad,
            supplier: m.proveedor,
            unit_cost: m.costoUnitario,
            total_cost: m.costoTotal
          }))
        );
      }

      if (reportData.avancePartidas.length > 0) {
        await supabase.from('report_progress').insert(
          reportData.avancePartidas.map(p => {
            const partida = partidasContrato.find(pc => pc.codigo === p.codigo);
            return {
              report_id: currentReportId,
              item_code: p.codigo,
              item_description: partida?.descripcion || '',
              progress_today: p.avanceHoy,
              progress_accumulated: p.avanceAcumulado,
              unit: partida?.unidad || '',
  ...
  const generateReport = () => {
    ...
    const report = {
      proyecto: project?.name || 'Villa Marina Fase 4',
      cliente: project?.client || 'Grupo VerdeAzul',
      fecha: currentDate,
      ...reportData,
      resumenDia: {
        costoPersonalCampo,
  ...
  const sendReport = async () => {
    ...
    const subject = 'Reporte Diario - Villa Marina - ' + currentDate;
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-4 shadow-lg">
        <div className="flex justify-between items-start">
          <div>
            <h1 className="text-xl font-bold">{project?.name || 'Villa Marina Fase 4'}</h1>
            <p className="text-blue-100 text-sm">{project?.client || 'Grupo VerdeAzul'}</p>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{reportData.materiales.length}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg">
        ...
        <div className="flex space-x-2 p-3 bg-gray-50">
          <button
            onClick={saveReport}
            disabled={saving}
            className="flex-1 bg-gray-600 text-white py-3 px-4 rounded-lg flex items-center justify-center font-medium hover:bg-gray-700 disabled:opacity-50"
          >
            {saving ? (
              <>
